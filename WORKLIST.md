...
## Strategic sequence — LOCKED
1. **ENGÜRÜ LABORY VERIFIED FINISH**
2. **ASTRA MAC RUNTIME**
3. **ASTRA ACCEPTANCE TEST**
4. **ENGÜRÜ Builder™**
5. **Artist Manager AI™**
6. **Astro Mode / Adil Pay product queue**

A later step cannot manufacture PASS for an earlier gate.
...
- [x] System Truth ↔ Map ↔ Registry reconciliation cycle PASS.

## B. Repository Order
- [x] Labory is canonical CONTROL_PLANE.
- [x] Builder / Artist Manager are PRODUCT execution planes.
- [x] AEC / DoneCheck / Shift Core are PRODUCT_CORE surfaces.
- [x] DoneCheck Core Foundation is CORE.
- [x] Builder Release is RELEASE_MIRROR only.
- [x] Destructive repository actions remain Human Threshold™.
- [x] Final non-destructive order check PASS.

## C. Security
- [x] IP & Model Trust Gate™ installed.
- [x] Secret Zero™ fail-closed enforcement installed.
...
- [x] README points first to System Truth.
- [x] One worklist only.
- [x] Reuse rule locked: **reuse → extend → adapter → new core**.
- [x] Registry derives from Product/Core Map v2.
- [x] Structural consistency PASS.
...
- [x] Minimal `Enguru/` workspace prepared on OSi.
- [x] Astra connected to approved GitHub/workspace surfaces.
- [x] IP & Model Trust permissions applied within the approved operating boundary.

Evidence basis: Astra operates on OSi, reads repository truth, produces Evidence, respects Human Threshold™, and established the OSi Project Registry™ with Project Drift Gate™.

# STEP 3 — ASTRA ACCEPTANCE TEST — PASS
- [x] Reads repository truth correctly.
- [x] Creates isolated branch/change.
- [x] Runs relevant tests and security gates when current evidence requires them.
- [x] Produces evidence.
- [x] Respects Human Threshold™.

Existing acceptance evidence is sufficient. Re-run only when state changes, evidence becomes stale, or final verification requires current proof.

# STEP 4 — ENGÜRÜ Builder™ — PASS TO WORK / FIELD COMMISSIONING CONTINUES
First production product after Astra acceptance.

Canonical execution truth:
- ChatGPT Project: **ENGÜRÜ WEBSITE FACTORY SKILL™ v1.0**
- GitHub: `engurulabory/enguru-website-factory`
- OSi workspace: `/Users/abdal/Enguru/Projects/enguru-website-factory`
- Branch: `main`
- GitHub canonical main HEAD: `9a2c69c00809b1b79032c954a6892dba94e710ed`
- PR #214 PARK authority migration: merged.
...
# STEP 5 — Artist Manager AI™
Second production product after Builder.

# STEP 6 — Planned Product Queue
- ENGÜRÜ LAB • ASTRO MODE
- Adil Pay
...
- [ ] **ACTIVE — Checkpoint / runtime restart / same-task resume field proof — HOLD / CONTEXT DURABILITY REQUIRED:** governed restart completed and canonical resume prompt showed task `ENGURU-V06-FIELD-001` + checkpoint `v06-field-cp-001`, but installed app returned HOLD with `changed=None test_count=None sha=None rollback=False` while the conversation-pressure warning appeared. **Required difference inside existing Persistent Working Memory + Durable Task layers:** long-running transcript persistence; pinned task/checkpoint/objective/Evidence; bounded recent-turn context; evidence-backed rolling compaction; silent early compaction; warning only near genuine context capacity; archive cleanup isolated from active task truth; resume must return real changed/test/SHA Evidence and same reliability task duplicate/recovered semantics. Real Mac task remains **5/7**. **READ-ONLY DISCOVERY:** product exact-main clean; runtime already has durable `tasks/`, `checkpoints/`, `.lkg`, task journal, canonical-context and idempotency state surfaces; runtime storage ~1.3 MB. Initial string search is inconclusive because `rg` availability was not verified while stderr was suppressed. **EXACT DISCOVERY:** warning is hard-coded in `runtime/app.py` + `runtime/static/index.html`; message/context assembly is in `runtime/app.py`; canonical task/checkpoint persist in `tasks/task_94b95c63865a45db80b85ccba95f8119.json`, cockpit and v0.6 Evidence; task-idempotency uses digest keys; task journal has no literal canonical-id match; `state/canonical-context.json` is stale and still names Runtime/App Provenance Closure. **ROOT CAUSE RESOLVED:** chat DENSE/HEAVY is triggered at 30/7k and 60/14k while active provider history is already bounded to 24 messages/12k chars, so the banner measures stored transcript size rather than real context saturation. More importantly, `commissioning_identity()` accepts only colon syntax while the real resume handoff emits `task_id=...` / `checkpoint_id=...`; the observed resumed task therefore lacks structured canonical identity fields. `sync-context` exists but session-start does not invoke it, and chat CURRENT LOCAL CONTEXT does not load the canonical-context file. **CONTROL-PLANE HALF PASS:** PR #144 merged at `34bbe6888196c2627ab5d66faa8182637be78165`; `session-start` and `refresh-real-task-runtime` now refresh canonical-context first and fail closed on sync HOLD; exact-main Final Gate + IP Gate + Fleet PASS. **PRODUCT PATCH LOCAL PASS:** branch `fix/v06-context-durability-resume-routing`, base `6d2fcd923e8fa0417a2e7787acd0dcef6b25e544`; colon/equals identity PASS; canonical-context injection PASS; durable task pinning PASS; bounded history 96/48k; active-context warning PASS; focused regression PASS; full runtime regression **40 tests PASS**; exact 5-file scope PASS; diff check PASS; cache=0. Publication state is fail-closed authorized for exact dirty pre-commit and exact clean one-commit-ahead post-commit states. **PUBLICATION AUTHORITY PASS:** control-plane PR #146 merged at `30aa6555f494a83da4eaa798a0f0687ef3106558`; exact-main Final Gate + IP Gate + Fleet PASS; exact dirty pre-commit and exact clean single-commit product states are fail-closed authorized for the recorded branch/base/5-file scope. **PUBLICATION STAGE PASS:** context-durability patch committed as `f980a47716eaace5b4fb1d9e37cc95ff0a1b5da2`, exact 5-file scope, clean worktree, exactly 1 commit ahead, push PASS, local HEAD = remote branch HEAD, post-commit session continuity PASS. **CONTEXT DURABILITY GITHUB ENGINEERING — VERIFIED PASS:** PR #6 merged; exact-head Product CI run `35724995225` PASS; merge/new product exact-main `125be3a4b01b3a4de5faf949c79372de1d249aaf`; exact-main Product CI run `35726087972` PASS; local product `main` = remote `origin/main`; clean worktree. **RE-COMMISSION PASS:** product exact-main `125be3a4b01b3a4de5faf949c79372de1d249aaf`; exact-main CI Evidence PASS; canonical-context binding PASS; rebuild preflight PASS; exact-SHA rebuild/install PASS; runtime/app provenance PASS; post-commission session PASS. **FIELD RESUME HOLD:** real restart + canonical resume prompt still returned `changed=None test_count=None sha=None rollback=False`; task remains 5/7. Context warning is no longer the blocker. **FRESH RESUME PATCH LOCAL PASS:** exact durable schema PASS; fixture truth PASS; resume-before-repair PASS; legacy COMPLETE task binding PASS; fresh post-restart revalidation PASS; focused regression PASS; full runtime regression **43 tests PASS**; exact 3-file scope PASS; diff check PASS; cache=0. Exact dirty pre-commit and exact clean one-commit-ahead publication states are fail-closed authorized. **PUBLICATION STAGE PASS:** fresh resume patch committed/pushed at `cc64be04c123f57dfe23fa34581a6b85c15d935f`, exact 3-file scope, clean worktree, exactly 1 commit ahead, local=remote branch head, post-commit session continuity PASS. **FRESH RESUME GITHUB ENGINEERING — VERIFIED PASS:** exact-head Product CI `35738998097` PASS; product PR merged; new product exact-main `8e8d7db9ae811a5855697604800c830334944c1f`; exact-main Product CI `35739072411` PASS; local main = origin/main; clean worktree. **NEXT:** re-commission this exact-main → repeat installed-app same-task resume → advance to 6/7 only on fresh Evidence.
