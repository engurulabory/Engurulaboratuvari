...
- [ ] **ACTIVE — Persistent continuity / Continuity Patch Repeatability Gate:** first restart field result exposed internal reliability identity drift while canonical task/checkpoint identity and repository state were preserved. Root cause was isolated to non-deterministic fallback idempotency in the existing reliability path. In-flight bounded patch is authorized on `fix/v06-durable-continuity-binding` at base/head `6f424c0b815d8c0cf8aa761124be3f743e412ee1` with exact scope `runtime/app.py`, `runtime/field_reliability.py`, `runtime/tests/test_field_continuity_binding.py`. Disposable diagnostic shows same reliability task, verified checkpoint, recovered=true, regression PASS and unchanged worktree. Current acceptance gate: TARGETED_REPEATABILITY=5/5_PASS + FULL_RUNTIME_REGRESSION=PASS + DIFF_CHECK=PASS + PATCH_SCOPE=3_FILES_PASS + PRODUCT_RUNTIME_CACHE_COUNT=0.
