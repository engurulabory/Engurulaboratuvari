...
- [ ] **ACTIVE — Real Mac task:** one real engineering task is executed through the actual Mac Engineering path; health-only evidence is insufficient. First disposable fixture attempt returned pre-execution HOLD with no source diff/result; field diagnosis found fixture/runtime-contract mismatch (new repo created after runtime inventory + root-level test layout + v0.8-style generic repair ask). Harness is being realigned to the already-verified v0.6 controlled canonical-state repair path; product core remains unchanged.
...
**Current single objective:** **Package 6 — Real Mac Engineering Task**.
