...
- [ ] **ACTIVE — Continuity patch exact-main Mac commissioning — HOLD / CI EVIDENCE REFRESH REQUIRED:** product exact-main `6d2fcd923e8fa0417a2e7787acd0dcef6b25e544`; canonical session-start PASS; rebuild preflight PASS; expected continuity runtime delta = 2 changed + 1 missing + 0 current-only; rebuild/install stopped before mutation on `CI_EVIDENCE_SOURCE_SHA_MISMATCH`. Root cause: local `package6-product-ci-exact-main.json` is stale relative to new exact-main Product CI run `35711368049`. **NEXT:** existing `verify-product-ci` → refreshed exact-main CI Evidence → rebuild/install → provenance.
- [ ] **Checkpoint / runtime restart / same-task resume field proof:** canonical task `ENGURU-V06-FIELD-001` + checkpoint `v06-field-cp-001` → restart runtime → resume same canonical task → verify same reliability task / duplicate+recovered semantics → regression → Evidence → promote real Mac task to 6/7 only on PASS.
...
**Current single objective:** **Package 6 — Continuity Patch Exact-Main Commissioning**.
