...
git -C "$PRODUCT_REPO" fetch origin --prune || hold "PRODUCT_FETCH_FAILED"
...
git -C "$PRODUCT_REPO" worktree add --detach "$WORKTREE" "$CANDIDATE_SHA" >/dev/null ||   hold "DISPOSABLE_WORKTREE_CREATE_FAILED"
...
for run_number in 1 2 3 4 5; do
  log="$RUN_DIR/targeted-$run_number.log"
  (
    cd "$WORKTREE"
    PYTHONPATH="$WORKTREE/runtime"       python3 -B -m unittest discover         -s runtime/tests         -p 'test_v07_long_running_reliability.py'         -v
  ) >"$log" 2>&1 || hold "TARGETED_RUN_${run_number}_FAILED"
  print "TARGETED_RUN_${run_number}=PASS"
done
